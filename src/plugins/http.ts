...
http.interceptors.request.use(response => {
    if (response.method && QS_METHOD.includes(response.method as Method)) {// 这里只处理post请求，根据自己情况修改
        response.data = qs.stringify(response.data);
    } else if (response.method && GET_METHOD.includes(response.method as Method)) {//设置GET的请求参数
